// RedisBridge
// Manage Redis subscriptions to fetch new songs for user loves and site adds.
// Sends new songs to shuffle.  Stores active subscriptions in LevelDB, which
// is backed up to S3 hourly.
//
...
// Load a subscriptions database backup from S3.
// Pull down subscriptionsBackup.tar.gz, save it to a file, extract the file,
// load the database and resolve.
...
// 'Scan' operation to pull all the keys from LevelDB
...
// Tar/gzip the subscriptions database and put it in S3.
...
// Only executed if there's no existing local subscriptions database, and
// also no backup in S3.
...
// The initialization function.  Connect redis, load the subscriptions database,
// subscribe to all of the saved channels in the subscriptions database.
...
// Subscribe to a redis channel, insert this channel in LevelDB.
...
// Unsubscribe from a redis channel, remove the key from LevelDB.
// Only used in testing, but an API route could be exposed if necessary.

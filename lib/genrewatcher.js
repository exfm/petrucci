...
function genreWatcher(){
	...
	sequence().then(function(next){
		...
		when.all(genres.map(function(genre){
			var p = when.defer();
			Petrucci.getTokens('genre:' + genre).then(function(tokens){
				getNewSongs(genre, tokens).then(p.resolve, p.reject);
			}, p.reject);
			return p.promise;
		})).then(next, error);
	});
...
function getNewSongs(genre, tokens){
	...
	when.all(tokens.map(function(token){
		...
		// find last time new songs were added
		// cloudsearch to get new songs for this genre
		// send new songs to shuffle
...
function getRecentLoves(genre, beginTimestamp){
	var d = when.defer(),
		size = 100,
...
function getLastNewSongs(token){
	...
	request
		...
		.end(function(res){
			...
			return d.resolve(res.body.last_new_songs);
		});
	return d.promise;
}

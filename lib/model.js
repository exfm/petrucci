    ...
    function addTokenToChannel(){
        sequence().then(function(next){
            self.get('petrucci', channel).then(function(existing){
                ...
                // take this out when mambo's Date is fixed in get
                petrucci.created = created;
                next(petrucci);
            });
        }).then(function(next){
...
Petrucci.unsubscribeFromPlayset = function(token){
    ...
    function removeTokenFromChannel(){
        sequence().then(function(next){
            self.get('petrucci', channel).then(function(existing){
                if (existing === null){
                    return d.reject(new Error('Token does not exist in this channel.'));
                }
                if (_.isEqual(existing.tokens, [token])){
                    // if this is the only token in the channel, destory the channel
                    return d.resolve(self.destroy(channel));
                }
